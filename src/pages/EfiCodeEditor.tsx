...
export default function EfiCodeEditor() {
  ...
  return <EfiCodeProvider globalCss={globalCss}>
        ...
        <div className="h-screen flex flex-col bg-background">
          ...
          <header className="h-14 border-b flex items-center justify-between px-4 bg-background">
            <div className="flex items-center gap-4">
              ...
              <Input value={siteName} onChange={e => setSiteName(e.target.value)} className="w-64 font-medium" placeholder="Nome do site" />
              
              {/* Responsiveness Toggles */}
              <ToggleGroup type="single" value={viewportSize} onValueChange={value => value && setViewportSize(value as ViewportSize)} className="border rounded-md">
            ...
            <div className="flex items-center gap-2">
              <EditorActions siteId={id} viewMode={viewMode} onViewModeChange={setViewMode} onSave={handleSave} onExport={handleExport} onGenerateCode={handleGenerateCode} />
            </div>
          </header>
          ...
          <div className="flex-1 flex overflow-hidden">
            ...
            backgroundColor: pageSettings.backgroundColor
              ...
              width: viewportWidths[viewportSize]
                  ...
                  <textarea value={codeContent} readOnly className="flex-1 w-full font-mono text-sm p-4 bg-secondary text-secondary-foreground rounded-b-lg resize-none focus:outline-none" spellCheck={false} />
                </div>}
            </main>
...
function EditorActions({
  ...
  return <>
      {/* Visual/Code Toggle */}
      <ToggleGroup type="single" value={viewMode} onValueChange={value => value && handleViewModeChange(value as ViewMode)} className="border rounded-md">
      ...
      {/* Undo/Redo */}
      <Button variant="ghost" size="icon" disabled={!canUndo} onClick={() => actions.history.undo()} title="Desfazer">
        <Undo2 className="h-4 w-4" />
      </Button>
      <Button variant="ghost" size="icon" disabled={!canRedo} onClick={() => actions.history.redo()} title="Refazer">
      ...
      {/* Save Button */}
      <Button size="sm" onClick={() => onSave(query)}>
